<x-admin.layouts.app :breadcrumb="$breadcrumb">
    <x-slot:pageTitle>{{ __('Page Settings') . ' : ' . __('Top Documents') }}</x-slot>
    ...
    <div class="row">
        <div class="col-12 col-lg-6">
            ...
            <form action="{{ route('admin.settings.popular-documents') }}" method="POST">
                @csrf
                @method('PUT')

                <div class="card">
                    <div class="card-header">
                        <b>{{ __('Page Settings') . ' : ' . __('Top Documents') }}</b>
                    </div>
                    <div class="card-body">
                        ...
                        <p>{{ __('URL') }} : <a href="{{ route('popular-documents') }}" target="_blank">{{ route('popular-documents') }}</a></p>

                        <x-admin.forms.input-text name="permalink" :label="__('Permalink')" :value="old('permalink') ?? $settings->get('permalink', 'top-documents')" />
                        <x-admin.forms.input-text name="title" :label="__('Page title') . ' (<title>...</title>)'" :value="old('title') ?? $settings->get('title', __('Top Documents'))" />
                        <x-admin.forms.input-text name="main_heading" :label="__('Main heading') . ' (<h1>...</h1>)'" :value="old('main_heading') ?? $settings->get('main_heading', __('Top Documents'))" />
